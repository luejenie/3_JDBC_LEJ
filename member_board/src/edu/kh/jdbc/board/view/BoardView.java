...
public class BoardView {
	...
	private void selectAllBoard() {
		...
		try {
			List<Board> boardList = bService.selectAllBoard(); //_ 서비스한테 목록 조회해서 가져오라고 한 뒤, List 목록에 저장
			// -> DAO에서 new ArrayList<>(); 구문으로 인해 반환되는 조회 결과는
			//    null이 될 수 없다!!!
			//_ 반환 되는 경우 : List, Board, int, Map
			//				 List는 array를 만들어서 가져오기 때문에 null인 경우가 거의 없음
			...
			if(boardList.isEmpty()) {  // 조회 결과가 없을 경우
				...
				for(Board b : boardList) {
				   //굴번호 | 제목[댓글수] | 작성자 | 작성시간  | 조회수
					// 3 | 샘플 제목3[4] | 유저삼 | 3시간 전 | 10
					System.out.printf(" %d  |  %s[%d]  | %s |  %-8s  | %d \n",
									b.getBoardNo(), 
									b.getBoardTitle(),
									b.getCommentCount(),
									b.getMemberName(),
	...
	private void selectBoard() {
		...
		try {
			...
			// 게시글 1개 -> vo Board			 //	게시글 번호,    로그인한 회원의 회원번호 
											 //              -> 자신의 글 조회수 증가X 위해서.
	...
	private void subBoardMenu(Board board) {
		...
		try {  //0번을 제외한 1~3번은 실행하자마자 게시판 메뉴가 아닌 댓글 작성한 게시글이 보여야함. -> 재귀호출
			...
			case 1 : insertComment(board.getBoardNo(), memberNo ); break;
									//_어떤 게시글인지,  작성자가 누구인지 알아야 함. 
												 //  _(로그인한 작성자 번호) _기니까 위에 변수로 선언
			...
			case 4 : case 5 :  // 4 또는 5 입력 시
				...
				// 4 또는 5를 입력한 회원이 게시글 작성자인 경우 v   __작성자가 아니면 default 값이 반환
				if(MainView.loginMember.getMemberNo() == board.getMemberNo()) {
					
					if(input == 4) {
						// 게시글 수정 호출
						updateBoard(board.getBoardNo());   //_몇 번 게시글인지만 알아야 되기 떄문에, 게시글 번호만 받아옴.
					}
					
					if(input == 5) {
						// 게시글 삭제 호출
						deleteBoard(board.getBoardNo());  //_몇 번 게시글인지만 알아야 되기 떄문에, 게시글 번호만 받아옴.
						// input = 0;
					}
			...
			// 댓글 등록, 수정, 삭제 선택 시 (1~3번)
			// 각각의 서비스 메서드 종료 후 다시 서브메뉴 메서드 호출(재귀호출)
			
			if(input > 0 && input < 5) {   //_1,2,3,4번 메뉴가 선택됐을 때마 아래 코드 실행  
											// <- 5번 실행 시  java.lang.NullPointerException 에러 해결방법  // or case5 밑에 input = 0;넣어도 가능
				try {
				...
				subBoardMenu(board);   //_재귀호출 :메서드가 자기 자신을 계속 호출 -> 잘못 작성하면 무한루프에 빠질 수 있음.
	...
	private void insertComment(int bNo, int Mno) { 
		                 //_변수 이름을 바꿔놔도 상관없음. 어떤것이 들어가는지가 더 중요! 위와 모양만 다를뿐 같음.
		...
		try {
			...
			// 내용 입력 받기
			//_ 이부분 아래를 따로 분리함.
			String content = inputContent();  // 빈 문자열. (빈칸을 참조하고 있다)  != null(참조안함)
	...
	private String inputContent() {
		...
		//_댓글을 여러줄 쓰기 위해
		String content = "";  // 빈 문자열. (빈칸을 참조하고 있다)  != null(참조안함)
		
		String input = null; // 참조하는 객체가 없음
		
		System.out.println("입력 종료 시 ($exit) 입력");
		while(true) {
			input = sc.nextLine();  //_while문 안에서 계속 입력 받음
			
			if(input.equals("$exit")) {
				break;
			}
			
			// 입력된 내용을 content에 누적 _누적할 떄마다 줄바꿈
			content += input + "\n";
	...
	private void insertBoard() {
		try {
			...
			Board board = new Board();
			board.setBoardTitle(boardTile);
			board.setBoardContent(boardContent);
			board.setMemberNo(MainView.loginMember.getMemberNo());
			
			int result = bService.insertBoard(board);
			// 0 또는 생성된 게시글 번호가 result에 담겨서 옴. (0 초과) _시퀀스가 1부터 시작 

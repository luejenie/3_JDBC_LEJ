...
public class MainDAO {
	...
	// _필드는 null 안 써도 null로 초기화 됨.
	private Statement stmt = null;
	private PreparedStatement pstmt = null;
	private ResultSet rs = null;
	
	private Properties prop = null;
	// Map<String, String> 제한, XML 파일을 읽기/쓰기에 특화
	// _SQL을 XML 파일에 써서 읽어올 때 사용
	...
	public int idDupCheck(Connection conn, String memberId) throws Exception {
		...
		try {
			...
			// n행 조회 -> while
			if(rs.next()) {
				// result = rs.getInt("COUNT(*)"); // 컬럼명 (가독성)
				result = rs.getInt(1); // 컬럼 순서 (개발식 순서)
